...
?>
        <div class="professor-callout">
            <div class="professor-callout__photo" style="background-image: url(
                <?php the_post_thumbnail_url("professorPortrait"); ?>)"></div>
                ...
                if ($relatedPrograms) { ?>
                    <p><?php echo wp_strip_all_tags(get_the_title()); ?> teaches:
                        <?php foreach ($relatedPrograms as $key => $program) {
                            ...
                            if (
                                $key != array_key_last($relatedPrograms) &&
                                count($relatedPrograms) > 1
                            ) {
                                echo ", ";
                            }
                        } ?>.
